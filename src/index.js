...
export default {
  ...
  mounted() {
    const { projectCode, language } = this.$props;
  ...
  methods: {
    setUserConfig() {
      const { projectCode, language, username, email, firstname, lastname, signUpAt, role, props } = this.$props;
 ...
 */
function isDOMReady() {
  return window?.document?.createElement !== undefined;
}
